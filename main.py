...
from controller.inspection import get_inspection_by_regno, get_image_inspection_by_regno, create_inspection, delete_inspection, update_inspection
...
@app.route("/inspections/<string:reg_no>", methods=["DELETE"])
@jwt_required()
def delete_inspection_route(reg_no):
    return delete_inspection(reg_no)

@app.route("/inspections/<string:reg_no>", methods=["PUT"])
@jwt_required()
def update_inspection_route(reg_no):
    return update_inspection(reg_no)
...
def test_dashboard_roda_2():
    try:
        result = db.session.execute(text('SELECT * FROM dashboard_roda_2'))
        columns = result.keys()
        rows = [dict(zip(columns, row)) for row in result.fetchall()]
...
def test_dashboard_roda_4():
    try:
        result = db.session.execute(text('SELECT * FROM dashboard_roda_4'))
        columns = result.keys()
        rows = [dict(zip(columns, row)) for row in result.fetchall()]
